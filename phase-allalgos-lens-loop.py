...
def main():
    # Define folder paths
    base_path = r"C:\Users\GOPAL\neurips dataset"
    conv_folder = os.path.join(base_path, "CONV")
    no_lens_folder = os.path.join(base_path, "NO LENS") 
    plane_lens_folder = os.path.join(base_path, "PLANE LENS")
    
    # Initialize results storage
    results = []
    
    # Process each index from 1 to 24
    for i in range(1, 25):
        print(f"\n{'='*50}")
        ...
        try:
            # Load files for this index
            no_lens_file = os.path.join(no_lens_folder, f"nolens{i}.h5")
            conv_file = os.path.join(conv_folder, f"conv{i}.h5")
            plane_lens_file = os.path.join(plane_lens_folder, f"pn{i}.h5")
            
            # Extract phase signals
            phi_no_lens, _, _, _ = extract_phase_signal(no_lens_file)
            phi_conv, _, _, _ = extract_phase_signal(conv_file)
            phi_plane, _, _, _ = extract_phase_signal(plane_lens_file)
            
            # Process with HDMD
            print("Running HDMD...")
            hdmd_no_lens, hdmd_time_no_lens = HDMD_algorithm(phi_no_lens)
            hdmd_conv, hdmd_time_conv = HDMD_algorithm(phi_conv)
            hdmd_plane, hdmd_time_plane = HDMD_algorithm(phi_plane)
            hdmd_avg_time = (hdmd_time_no_lens + hdmd_time_conv + hdmd_time_plane) / 3
            
            row.update({
                'HDMD_NoLens': hdmd_no_lens,
                'HDMD_Conv': hdmd_conv,
                'HDMD_PN': hdmd_plane,
                'HDMD_Time': hdmd_avg_time
            })
            
            # Process with EMD
            print("Running EMD...")
            emd_no_lens, emd_time_no_lens = EMD_algorithm(phi_no_lens)
            emd_conv, emd_time_conv = EMD_algorithm(phi_conv)
            emd_plane, emd_time_plane = EMD_algorithm(phi_plane)
            emd_avg_time = (emd_time_no_lens + emd_time_conv + emd_time_plane) / 3
            
            row.update({
                'EMD_NoLens': emd_no_lens,
                'EMD_Conv': emd_conv,
                'EMD_PN': emd_plane,
                'EMD_Time': emd_avg_time
            })
            
            # Process with VMD
            print("Running VMD...")
            vmd_no_lens, vmd_time_no_lens = VMD_algorithm(phi_no_lens)
            vmd_conv, vmd_time_conv = VMD_algorithm(phi_conv)
            vmd_plane, vmd_time_plane = VMD_algorithm(phi_plane)
            vmd_avg_time = (vmd_time_no_lens + vmd_time_conv + vmd_time_plane) / 3
            
            row.update({
                'VMD_NoLens': vmd_no_lens,
                'VMD_Conv': vmd_conv,
                'VMD_PN': vmd_plane,
                'VMD_Time': vmd_avg_time
            })
            
            # Process with DWT
            print("Running DWT...")
            dwt_no_lens, dwt_time_no_lens = DWT_algorithm(phi_no_lens)
            dwt_conv, dwt_time_conv = DWT_algorithm(phi_conv)
            dwt_plane, dwt_time_plane = DWT_algorithm(phi_plane)
            dwt_avg_time = (dwt_time_no_lens + dwt_time_conv + dwt_time_plane) / 3
            
            row.update({
                'DWT_NoLens': dwt_no_lens,
                'DWT_Conv': dwt_conv,
                'DWT_PN': dwt_plane,
                'DWT_Time': dwt_avg_time
            })
            ...
            for algo in ['HDMD', 'EMD', 'VMD', 'DWT']:
                for case in ['NoLens', 'Conv', 'PN']:
                    empty_row[f'{algo}_{case}'] = np.nan
    ...
    columns_order = ['Index']
    for algo in ['HDMD', 'EMD', 'VMD', 'DWT']:
        columns_order.extend([f'{algo}_NoLens', f'{algo}_Conv', f'{algo}_PN', f'{algo}_Time'])
